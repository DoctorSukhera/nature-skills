...
class OpenAIResponsesClient:
    ...
    def _retry_delay(response, attempt: int) -> float:
        # Prefer the server-provided Retry-After header. Some rate-limit
        # messages also include text such as "Please try again in 4.092s".
        raw = (response.headers.get("Retry-After") or "").strip()
        if raw:
            try:
                return max(0.5, float(raw)) + random.uniform(0.25, 0.9)
            except ValueError:
                pass
        try:
            body = response.json()
            msg = str((body.get("error") or {}).get("message") or "")
            m = re.search(r"try again in\s+([0-9.]+)s", msg, flags=re.I)
            if m:
                return max(0.5, float(m.group(1))) + random.uniform(0.25, 0.9)
        except Exception:
            pass
        # Exponential backoff with jitter for temporary errors when the
        # server gives no reset hint.
        return min(60.0, (2 ** attempt) + random.uniform(0.25, 1.0))
    ...
    def _base_payload(self, *, instructions: str, schema_name: str, schema: dict[str, Any], max_output_tokens: int, web_search: bool) -> dict[str, Any]:
        payload: dict[str, Any] = {
            ...
            "store": False,
            "prompt_cache_key": "nanomax-nature-editor-v10",
        }
        if web_search:
