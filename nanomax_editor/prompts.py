...
def journal_discovery_prompt(manuscript: str, publishing_preference: str, support_context: str = "", priorities: str = "") -> str:
    return f"""
TASK: Recommend the best-fitting Nature Portfolio journals for the manuscript BEFORE formatting begins.

PUBLISHING PREFERENCE: {publishing_preference}
AUTHOR PRIORITIES: {priorities or 'No additional priorities.'}

REQUIREMENTS
1. Consider Nature Portfolio journals only. Do not recommend unrelated Springer/BMC journals.
2. Rank primarily by scientific scope fit, study design, evidence strength, novelty level and article-type compatibility — NOT by Impact Factor.
3. Apply the publishing preference strictly:
   - 'Fully Open Access only' => return only fully open-access journals.
   - 'Hybrid only' => return only journals that offer traditional subscription publishing plus an optional Gold OA route for the recommended primary research article type.
   - 'Either' => both are eligible.
4. Use LIVE WEB SEARCH and FIRST-PARTY Nature/Springer Nature pages for aims/scope, article types, publishing model, APC and journal metrics.
5. For Journal Impact Factor, 5-year JIF, median submission-to-first-editorial-decision, median submission-to-acceptance, SNIP and SJR, use the journal's official current Journal Metrics page when available.
6. 'First editorial decision' must mean the publisher's metric from submission until the manuscript is either sent for peer review or rejected. Do not mislabel it as peer-review time.
7. Do not invent an acceptance rate. If the journal does not officially publish one, return 'Not publicly reported'.
8. APC must be current and journal/article-type appropriate when an official page reports it. If multiple currencies are listed, include them in the APC string.
9. If an official metric cannot be verified, use 0 for numeric metric fields and explain the absence in publishing_options or main_risk.
10. Return 3–5 candidates when possible. Include at least one realistic fit-focused option rather than only aspirational high-impact journals.
11. Explain why each journal fits and the main reason it might reject the manuscript.
12. metrics_checked_date must state the date used for the live check.
13. official_sources must contain direct first-party URLs/pages used for that candidate.
14. Do not imply that a high fit score is an acceptance probability.
