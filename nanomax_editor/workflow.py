...
class NatureReadyWorkflow:
    def __init__(self, client: OpenAIResponsesClient, skill_text: str):
        self.client = client
        # Full Nature Skills are valuable for manuscript production, but they
        # are unnecessarily expensive for the journal-discovery/profile calls.
        # Keep those calls compact to avoid consuming ~100k instruction tokens
        # before manuscript production even starts.
        self.instructions = with_skills(skill_text)
        self.journal_instructions = (
            "You are NanoMax Journal Discovery. Use current first-party Nature/Springer Nature "
            "sources. Rank journals by scientific scope and evidence fit, not prestige alone. "
            "Never invent journal metrics, APCs, acceptance rates, article types, or formatting rules. "
            "Return only information supported by current official sources and the requested JSON schema."
        )
    ...
    def recommend_journals(self, *, manuscript: str, publishing_preference: str, support_context: str = "", priorities: str = "", state: WorkflowState) -> dict:
        r = self.client.structured(
            instructions=self.journal_instructions,
