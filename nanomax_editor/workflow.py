...
from .prompts import (
    with_skills, journal_profile_prompt, figure_audit_prompt, transform_prompt,
    reference_audit_prompt, finalize_prompt, review_prompt,
)
from .schemas import JOURNAL_PROFILE_SCHEMA, FIGURE_AUDIT_SCHEMA, MANUSCRIPT_SCHEMA, REFERENCE_AUDIT_SCHEMA, REVIEW_SCHEMA
...
class NatureReadyWorkflow:
    ...
    def journal_profile(self, *, journal: str, article_type: str, state: WorkflowState) -> dict:
        r = self.client.structured(
            ...
            schema_name="nanomax_journal_profile_v7",
    ...
    def audit_figures(self, *, manuscript: str, images: list[dict], journal_profile: dict, state: WorkflowState) -> dict:
        ...
        try:
            r = self.client.structured_with_images(
                ...
                images=images[:16],
                schema_name="nanomax_figure_audit_v7",
                schema=FIGURE_AUDIT_SCHEMA,
                max_output_tokens=28000,
            )
            state.figure_audit = self._record(state, r)
        except Exception as exc:
            # Vision is helpful but should not make the entire manuscript pipeline unusable.
            state.figure_audit = {
                "summary": f"Automated visual audit unavailable; figure planning will use manuscript captions and asset order. Technical note: {exc}",
                "assets": [
                    {"asset_index":i, "detected_role":"other", "content_summary":img.get("name",f"asset {i}"),
                     ...
                     "reason":"Visual audit unavailable; preserve source evidence unless a later safe classification is possible.", "suggested_legend":""}
                    for i,img in enumerate(images,1)
                ]
            }
        return state.figure_audit

    def transform(self, *, manuscript: str, tables: list[dict], figure_audit: dict, journal_profile: dict, journal: str, article_type: str, priorities: str, state: WorkflowState) -> dict:
        r = self.client.structured(
            instructions=self.instructions,
            input_text=transform_prompt(manuscript, tables, figure_audit, journal_profile, journal, article_type, priorities),
            schema_name="nanomax_manuscript_draft_v7",
    ...
    def audit_references(self, *, manuscript: str, references: list[str], journal: str, state: WorkflowState) -> dict:
        r = self.client.structured(
            ...
            schema_name="nanomax_reference_audit_v7",
    ...
    def finalize(self, *, draft: dict, reference_audit: dict, journal_profile: dict, figure_audit: dict, journal: str, article_type: str, state: WorkflowState) -> dict:
        r = self.client.structured(
            instructions=self.instructions,
            input_text=finalize_prompt(draft, reference_audit, journal_profile, figure_audit, journal, article_type),
            schema_name="nanomax_final_manuscript_v7",
            schema=MANUSCRIPT_SCHEMA,
            max_output_tokens=100000,
        )
    ...
    def review(self, *, transformed: dict, journal: str, article_type: str, state: WorkflowState) -> dict:
        r = self.client.structured(
            ...
            schema_name="nanomax_submission_gate_v7",
