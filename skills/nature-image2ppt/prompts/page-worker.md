...
1. Keep `manifest.json` as the only page build source. Add arrow fields directly to standard `shapes[]`; do not create an Image2PPT plan, OCR copy, review manifest, or controller state.
...
5. Fix `manifest.json` or assets, rebuild, and repeat until the render matches. Then run:

   `python {{SKILL_ROOT}}/scripts/run_image2ppt_qa.py {{PAGE_DIR}} --visual-review-status reviewed --visual-review-notes "<specific observations checked>"`
