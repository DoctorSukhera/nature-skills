...
def reset_result():
    for k in ["result","redrawn","graphical_abstract","synthetic_previews"]:
...
st.markdown(f"""<div class='hero'><div class='k'>NanoMax Lab • Journal-Contract Manuscript Production + Research Completion • v{APP_VERSION}</div><div class='t'>NanoMax Nature Editor</div><div class='s'>Builds an evidence-locked manuscript using the selected journal's live article outline, exact heading/subheading rules, figures, tables, citations and declarations. A separate Research Completion Engine designs missing experiments/analyses and can generate clearly labelled synthetic previews for planning only.</div><div><span class='pill'>Exact journal outline</span><span class='pill'>One master DOCX</span><span class='pill'>Evidence-locked submission</span><span class='pill'>Nature Skills router</span><span class='pill'>Experiment designer</span><span class='pill'>Synthetic Preview Lab</span></div></div>""",unsafe_allow_html=True)
...
priorities=st.text_area("Author priorities (optional)",placeholder="e.g. preserve all valid analyses; target Nature Cancer Article; use journal-correct headings; improve conceptual figures")
...
st.header("2. Build journal-contract manuscript")
if st.button("Run NanoMax full workflow",type="primary",disabled=not bool(up),use_container_width=True):
    ...
    try:
        ...
        with st.status("NanoMax is building the journal-contract manuscript...",expanded=True) as status:
            st.write(f"1/{total} — Retrieving current official journal/article-type rules and exact outline")
        ...
        st.session_state.result={"profile":profile,"figure_audit":figuraudit,"draft":draft,"refaudit":refaudit,"final":final,"review":review,"research":research}
    except Exception as e:
        st.exception(e)
