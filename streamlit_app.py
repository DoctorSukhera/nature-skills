...
from nanomax_editor.workflow import NatureReadyWorkflow, WorkflowState, WORKFLOW_VERSION
...
# Fail clearly if Streamlit is running a mixed-version deployment (e.g. new app + old workflow.py).
if WORKFLOW_VERSION != APP_VERSION:
    st.error(
        f"Mixed NanoMax deployment detected: app v{APP_VERSION} but workflow v{WORKFLOW_VERSION}. "
        "Replace the complete nanomax_editor folder and reboot the Streamlit app."
    )
    st.stop()
st.markdown("""
<style>
